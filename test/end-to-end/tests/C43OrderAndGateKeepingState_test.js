...
Feature('Solicitor - Manage order, Gatekeeping & Service of application tests @debug');
...
Scenario('As a Case Manager Confidentiality Check @regression-suite @debug', async({ I }) => {
  await I.loginAsCaseManager();
  await I.searchForCasesWithId(caseId);
  await I.caseManagerConfidentialityCheck();
}).retry(testConfig.TestRetryScenarios);

Scenario('As a court admin Verify SOA and Task tab after Confidentiality check @regression-suite @debug', async({ I }) => {
  await I.loginAsSwanseaCourtAdmin();
  await I.searchForCasesWithId(caseId);
