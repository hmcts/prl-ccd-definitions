...
Scenario(
  'CA Manage Order Upload Order Serve order Personally @regression-suite @ptl-regression',
  async({ I }) => {
...
Scenario(
  'CA Manage Order Upload Order - For Judge review - @regression-suite @ptl-regression',
  async({ I }) => {
...
Scenario('As a court admin Create Order C43 @regression-suite @ptl-regression', async({ I }) => {
  await I.loginAsSwanseaCourtAdmin();
  await I.searchForCasesWithId(caseId);
  await I.createOrderC43AndSendToCaseManager();
}).retry(testConfig.TestRetryScenarios);

Scenario('As a Case manager edit & approve a draft order @regression-suite @ptl-regression', async({ I }) => {
